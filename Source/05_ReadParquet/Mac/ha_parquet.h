/* Copyright (c) 2024, Study Project.
   READ_PARQUET storage engine — reads a local .parquet file as a MySQL table.
...
#include "sql/handler.h"   // handler, Handler_share, handlerton
...
class Parquet_share : public Handler_share {
 public:
  THR_LOCK lock;                  /* MySQL table-level lock object            */
...
class ha_parquet : public handler {
 public:
  ...
  int write_row(uchar *)                override { return HA_ERR_WRONG_COMMAND; }
  int update_row(const uchar *, uchar *) override { return HA_ERR_WRONG_COMMAND; }
 ...
 private:
  ...
  /**
   * Fill one MySQL row buffer from row @p row_idx of arrow_table.
